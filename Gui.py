...
class RSSWindow:
    ...
    def __init__(self, ShellInstance):
        ...
        self.mainframe = ttk.Frame(
            self.root, width=300, height=300, padding="5 5 5 5")
        ...
        generator_output_box = ttk.Frame(
            self.mainframe, borderwidth=2, relief="sunken", width=400, height=400)
        server_output_box = ttk.Frame(
            self.mainframe, borderwidth=2, relief="sunken", width=400, height=400)
        options_box = ttk.Frame(self.mainframe, width=200, height=400)

        Label(self.mainframe, text="Options: ").grid(
            row=0, column=0, sticky="news", pady=5, padx=5)
        Label(self.mainframe, text="Generator Output: ").grid(
            row=0, column=1, sticky="news", pady=5, padx=5)
        Label(self.mainframe, text="Server Output: ").grid(
            row=0, column=2, sticky="news", pady=5, padx=5)

        options_box.grid(row=1, column=0, sticky="news", pady=5, padx=5)
        generator_output_box.grid(
            row=1, column=1, sticky="news", pady=5, padx=5)
        ...
        generator_output_y_scrollbar.config(
            command=self.generator_output.yview)
        ...
        generator_output_x_scrollbar.config(
            command=self.generator_output.xview)
        ...
        server_output_x_scrollbar = Scrollbar(
            server_output_box, orient=HORIZONTAL)
        ...
        port = StringVar()
        Label(options_box, text="Server Options:").grid(
            row=1, column=0, sticky="news", pady=5, padx=5, columnspan=2)
            ...
            try:
                ...
                if 1 <= port_num <= 65535:
                    ...
                    messagebox.showerror(
                        "Invalid Port Number", "Port numbers must be between 1 and 65535")
            except ValueError:
                messagebox.showerror("Invalid Port Number",
                                     "Port numbers must be whole numbers")

        def open_browser_gui():
            if self.shell.server is not None:
                print(self.shell.current_port)
                webbrowser.open("http://localhost:" +
                                str(self.shell.current_port))
            else:
                messagebox.showerror(
                    "Server Not Running", "You can't open the web interface because the server isn't running!")
        ...
        ttk.Button(options_box, text="Start", command=start_server).grid(
